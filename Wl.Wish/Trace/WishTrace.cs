...
namespace Wl.Wish
{
    public static class WishTrace
    {
        ...
        /// 统一日志锁名称
        /// </summary>
        const string LockName = "WishTraceLock";
        ...
        /// <summary>
        /// 记录WishException日志时需要执行的任务
        /// </summary>
        public static Action<WishException> OnWishExceptionFunc;

        /// <summary>
        /// 执行所有日志记录操作时执行的任务（发生在Wl.Wish记录日志之后）
        /// </summary>
        ...
        internal static void Open()
        {
            ...
            {
                var logDir = System.IO.Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, "App_Data", "WishTraceLog");
                ...
                string logFile = Path.Combine(logDir, string.Format("WlWishTrace-{0}.log", DateTime.Now.ToString("yyyyMMdd")));
        ...
        #region WishException 相关日志

        /// <summary>
        /// WishException 日志
        /// </summary>
        /// <param name="ex"></param>
        public static void WishExceptionLog(WishException ex)
        {
            if (!Config.IsDebug)
            {
                return;
